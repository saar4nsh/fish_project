...
def calculate_mask_metrics(pred_mask, gt_mask):
    """
    Calculate Dice and IoU metrics for binary masks
    
    Args:
        pred_mask: Predicted binary mask (torch tensor)
        gt_mask: Ground truth binary mask (torch tensor)
    
    Returns:
        dict: Dictionary containing Dice and IoU scores
    """
    pred_mask = (pred_mask > 0.5).type(torch.float32)
    gt_mask = (gt_mask > 0.5).type(torch.float32)

    dice_metric = Dice(gt_mask, pred_mask)
    iou_metric = IOU(gt_mask, pred_mask)

    dice_score = dice_metric.calculate()
    iou_score = iou_metric.calculate()
...
def create_prediction_masks(predictions, image_shape, confidence_threshold=0.5):
    """
    Create a combined mask from model predictions
    
    Args:
        predictions: Model predictions containing masks, scores, and labels
        image_shape: Shape of the original image (H, W)
        confidence_threshold: Minimum confidence score to include prediction
    
    Returns:
        numpy array: Combined prediction mask
    """
    pred_mask = np.zeroes(image_shape, dtype=np.uint8)

    for i, (mask, score, label) in enumerate(zip(predictions['masks'], predictions['scores'], predictions['labels'])):
        if score > confidence_threshold:
            if isinstance(mask, torch.Tensor):
                # Convert tensor to numpy and resize if necessary
            ...
            mask_np = np.maximum(pred_mask, (mask_np > 0.5).astype(np.uint8))
    return pred_mask   
...
def evaluate_with_ground_truth_and_metrics(model, test_dataset, confidence_threshold=0.5):
    ...
    all_dice_scores = []
    all_iou_scores = []
    class_wise_metrics = defaultdict(list)
    
    print("\nEvaluation against test ground truth with metrics:")
    
    with torch.no_grad():
        for batch_idx, (images, gene_patches, targets) in enumerate(tqdm(test_loader)):
            patch_images = []
            patch_targets = []
            
            for image, target in zip(images, targets):
                patch_images.extend([torch.squeeze(img).to(DEVICE) for img in image])
                patch_targets.extend([{k: v.to(DEVICE) for k, v in t.items()} for t in target])
                torch.cuda.empty_cache()
            ...
            # Process each patch
            for patch_idx, (prediction, target, gene_patch) in enumerate(zip(predictions, patch_targets, gene_patches[0])):
                # Get image shape from gene patch
                image_shape = (gene_patch.shape[1], gene_patch.shape[2])  # H, W
                
                # Create prediction mask
                pred_mask = create_prediction_masks(prediction, image_shape, confidence_threshold)
                
                # Create ground truth mask from target
                gt_masks = target["masks"].cpu().numpy()
                gt_labels = target["labels"].cpu().numpy()
                
                if len(gt_masks) > 0:
                    # Combine all ground truth masks
                    gt_combined = np.zeros(image_shape, dtype=np.uint8)
                    for mask in gt_masks:
                        gt_combined = np.maximum(gt_combined, mask.astype(np.uint8))
                    
                    # Calculate overall metrics
                    metrics = calculate_mask_metrics(pred_mask, gt_combined)
                    ...
                    for gt_label in np.unique(gt_labels):
                        class_name = LABEL_TO_CLASS.get(gt_label.item(), "unknown")
                        
                        # Create GT mask for this class
                        gt_class_mask = np.zeros(image_shape, dtype=np.uint8)
                        for i, label in enumerate(gt_labels):
                            if label == gt_label:
                                gt_class_mask = np.maximum(gt_class_mask, gt_masks[i].astype(np.uint8))
                        
                        # Create prediction mask for this class
                        pred_class_mask = np.zeros(image_shape, dtype=np.uint8)
                        for i, (label, score, mask) in enumerate(zip(pred_labels, pred_scores, pred_masks)):
                            if label == gt_label and score > confidence_threshold:
                                mask_resized = cv2.resize(mask.squeeze(), (image_shape[1], image_shape[0]))
                                pred_class_mask = np.maximum(pred_class_mask, (mask_resized > 0.5).astype(np.uint8))
                        
                        # Calculate metrics for this class
                        if np.sum(gt_class_mask) > 0:  # Only calculate if GT has this class
                            class_metrics = calculate_mask_metrics(pred_class_mask, gt_class_mask)
                            class_wise_metrics[class_name].append(class_metrics)
                ...
                print(f"\nBatch {batch_idx}, Patch {patch_idx}:")
                print(f"  Predicted counts: {dict(pred_counts)}")
    ...
    print("\nClass-wise Metrics:")
    for class_name, metrics_list in class_wise_metrics.items():
        if metrics_list:
...
evaluate_with_ground_truth_and_metrics(model, test_dataset)